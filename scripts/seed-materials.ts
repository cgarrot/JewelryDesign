...
// Seed data: ~50 materials across all categories (same as API endpoint)
const seedMaterials = [
  // Material Category (~8)
  {
    name: 'Gold',
...
async function seed() {
  ...
  for (const materialData of seedMaterials) {
    try {
      ...
      if (existing) {
        console.log(`⏭️  Skipped: ${materialData.name} (already exists)`);
        skipped++;
        continue;
      }
      ...
      console.log(`✅ Created: ${materialData.name} (${materialData.category})`);
      created++;
    } catch (error) {
      console.error(`❌ Error creating ${materialData.name}:`, error);
    }
  }

  console.log(`\n✨ Seed complete!`);
  console.log(`   Created: ${created} materials`);
  console.log(`   Skipped: ${skipped} materials (already existed)`);
  console.log(`   Total: ${seedMaterials.length} materials`);
}
