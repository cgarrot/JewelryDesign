...
import { ArrowLeft, Gem, FileDown, ChevronLeft, ChevronRight, MessageSquare } from 'lucide-react';
  ...
  loading: () => (
    <div className="flex items-center justify-center h-full">
      <div className="text-center">
        <Gem className="h-8 w-8 mx-auto mb-2 text-gray-400 animate-pulse" />
...
const ReferenceImageManager = dynamic(() => import('@/components/canvas/ReferenceImageManager').then(mod => ({ default: mod.ReferenceImageManager })), {
  loading: () => (
...
export default function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [activeTab, setActiveTab] = useState<'images' | 'views' | 'drawing'>('images');
  ...
  const fetchProject = async () => {
    ...
    try {
      ...
      toastError(error instanceof Error ? error.message : 'Failed to fetch project');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      ...
      toastError(error instanceof Error ? error.message : 'Failed to send message');
    } finally {
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({ 
          ...
          prompt,
          referenceImageIds: selectedReferenceImageIds.length > 0 
      ...
      toastError(error instanceof Error ? error.message : 'Failed to generate image');
    } finally {
  ...
  const handleExportProject = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `project-${project.name.replace(/[^a-z0-9]/gi, '-')}-${Date.now()}.json`;
      ...
      toastError(error instanceof Error ? error.message : 'Failed to export project');
    }
  };
  ...
  const handleSaveReferenceImage = async (imageData: string) => {
    ...
    try {
      const response = await fetch(`/api/projects/${project.id}/reference-images`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      toastError(error instanceof Error ? error.message : 'Failed to save reference image');
    }
  };

  const handleSaveAnnotatedImage = async (annotatedImageData: string, annotations: string, colorDescriptions?: Record<string, string>) => {
    if (!project) return;

    try {
      // Save the annotated image as a new reference image
      const refResponse = await fetch(`/api/projects/${project.id}/reference-images`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
      ...
      toastError(error instanceof Error ? error.message : 'Failed to process annotated image');
    } finally {
      setChatLoading(false);
      setImageLoading(false);
    }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Ctrl/Cmd + G to generate image
        ...
        e.preventDefault();
        if (!imageLoading && !chatLoading && project && project.messages && project.messages.length > 0) {
          handleGenerateImage();
        }
      }
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {chatCollapsed ? (
          ...
          <div className="flex flex-col relative" style={{ width: `${chatWidth}px`, minWidth: '300px', maxWidth: '60%' }}>
            <Button
              variant="ghost"
              size="sm"
            ...
            <div
              ...
              onMouseDown={(e) => {
                ...
                const handleMove = (moveEvent: MouseEvent) => {
                  ...
                  const newWidth = Math.max(300, Math.min(window.innerWidth * 0.6, startWidth + diff));
          ...
          <div className="flex-1 overflow-hidden">
            {activeTab === 'images' && (
              <ImageCanvas
                ...
                disabled={chatLoading || !project.messages || project.messages.length === 0}
              />
            )}
            {activeTab === 'views' && (
              <MultiViewCanvas
